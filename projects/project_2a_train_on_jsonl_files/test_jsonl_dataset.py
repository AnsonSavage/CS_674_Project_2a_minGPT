...
class TestJSONLDataset(unittest.TestCase):
    ...
    def test_length(self):
        """
        Test that the length of the dataset is correct when loading /nobackup/archive/usr/dw87/pile_data_10.jsonl
        """

        path_to_jsonl = '/nobackup/archive/usr/dw87/pile_data_10.jsonl'
        dataset = JSONLDataset(path_to_jsonl)
        self.assertEqual(len(dataset), 7023114)
    
    def test_item_0(self):
        """
        Test that the first item in the dataset is correct when loading /nobackup/archive/usr/dw87/pile_data_10.jsonl
        """

        path_to_jsonl = '/nobackup/archive/usr/dw87/pile_data_10.jsonl'
        dataset = JSONLDataset(path_to_jsonl, length=7023114)
    ...
    def test_item_1(self):
        """
        Test that the second item in the dataset is correct when loading /nobackup/archive/usr/dw87/pile_data_10.jsonl
        """
